...
def route_after_classify(state: AgentState) -> str:
    ...
    Unknown or missing routes safely default to ``answer``.
    """
    destinations = {
        "simple": "answer",
        "tool": "tool",
    ...
    return destinations.get(state.get("route", ""), "answer")
...
def route_after_retry(state: AgentState) -> str:
    ...
    try:
        return "tool" if int(attempt) < int(max_attempts) else "dead_letter"
